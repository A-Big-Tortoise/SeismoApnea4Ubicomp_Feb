...
def analyze_and_save_fold_distributions():
    ...
    save_dir = f'Data/fold_data_p{109}_MTL_60s_bedroom/'
    ...
    for fold in range(1, 5):
        ...
        for patient_file in patient_files:
            ...
            if id2room.get(int(patient_id)) != fold:
...
if __name__ == "__main__":
    df = pd.read_excel('Code/SleepLab.xlsx', sheet_name='Logs')
    keys = id2fold.keys()
    df_filtered = df[df['ID'].isin(keys)]
    id2room = dict(zip(df_filtered['ID'], df_filtered['Room']))
