...
def modify_magnitude_multi_freq_noise(
	...
	n_centers=1,           # centers=None 时随机采样中心频率个数
...
if __name__ == "__main__":
	url = 'https://sensorserver.engr.uga.edu:3000/d/BSGRS3dRoom4/sleep-lab-tri-axis-monitoring-room-4?orgId=1&var-mac=b8:27:eb:64:49:4d&var-name=vitalsigns&from=1742696515000&to=1742696575000'
	...
	plt.tight_layout()
	plt.savefig(f'/home/jiayu/SeismoApnea4Ubicomp_Feb/Infos/fig_preprocess/X_preprocess_large', bbox_inches='tight', dpi=300)
	
	

	# fig, axes = plt.subplots(row, 1, figsize=(12, 2.35 *  row), sharex=True)
	# axes[0].plot(time, Y, label='Y (raw)', linewidth=lw)
	# axes[1].plot(time, signal_process(Y), label='Y (filt)', linewidth=lw)
	# # axes[1].plot(time, modify_magnitude_multi_freq_noise(signal_process(Y), fs=10, band=(0.3, 0.75)), label='Y (freq perturbation)', linewidth=lw)
	# axes[0].legend()
	# axes[1].legend()
	# axes[0].set_title('(a) Raw Y-axis seismic signal')
	# axes[1].set_title('(b) Filtered Y-axis signal')
	# axes[1].set_xlabel('Time [s]')
	# plt.tight_layout()
	# plt.savefig(f'/home/jiayu/SeismoApnea4Ubicomp_Feb/Infos/fig_preprocess/Y_preprocess_large', bbox_inches='tight', dpi=300)
