...
class TSTiEncoder(nn.Module):  #i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
                 ...
                 key_padding_mask='auto', res_attention=True, pre_norm=False,
    ...
    def forward(self, x) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        ...
        # # --- reshape input for linear ---
        # B, C, P, L = x.shape                    # batch, channel, patch_num, patch_len
        # x = self.W_P(x.reshape(B*C*P, L)).reshape(B, C, P, -1)   # [B, C, P, d_model]

        # # --- combine with positional encoding ---
        # u = x.reshape(B*C, P, -1)               # [B*C, patch_num, d_model]
        # W_pos = self.W_pos.unsqueeze(0).expand(u.size(0), u.size(1), u.size(2))  # [B*C, patch_num, d_model]
        # u = self.dropout(u + W_pos)


        # ================== 
        # use [cls] token
        # bs_nvars = x.shape[0] * x.shape[1]
        # seq_len = x.shape[2]

        # u = torch.reshape(x, (bs_nvars, seq_len, x.shape[3]))  # [bs * nvars, patch_num, d_model]

        # cls_token = self.cls_token.expand(bs_nvars, -1, -1)  # [bs * nvars, 1, d_model]
        # u = torch.cat((cls_token, u), dim=1)  # [bs * nvars, 1 + patch_num, d_model]

        # # positional encoding needs to be updated accordingly
        # W_pos = self.W_pos
        # if W_pos.shape[0] == seq_len:  # original length
        #     W_pos = torch.cat([torch.zeros(1, W_pos.shape[1], device=W_pos.device), W_pos], dim=0)  # [1 + patch_num, d_model]
        
        # u = self.dropout(u + W_pos)
        # ================== 

        # Encoder
        # print(f'shape before encoder: {u.shape}') 

        z = self.encoder(u)                                                      # z: [bs * nvars x patch_num x d_model]
        # print(f'shape after encoder: {z.shape}')
        z = torch.reshape(z, (-1,n_vars,z.shape[-2],z.shape[-1]))                # z: [bs x nvars x patch_num x d_model]
        # print(f'shape after reshape: {z.shape}')
        z = z.permute(0,1,3,2)                                                   # z: [bs x nvars x d_model x patch_num]
        # print(f'shape after permute: {z.shape}')
