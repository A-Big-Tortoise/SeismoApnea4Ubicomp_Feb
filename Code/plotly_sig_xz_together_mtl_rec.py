...
def plot_person_level_results(y_true_list, y_pred_list,
		...
		# build annotation strings: "xx.xx%\ncount"
...
def inference_REC(X_concat, Y_concat, model, device, step_sig, threshold):
	...
	with torch.no_grad():
		for start in range(0, len(segments_tensor), batch_size):
			...
			outputs1_stage, outputs1_apnea, _, _, _, rec_tho, rec_abd = model(batch)
			outputs2_stage, outputs2_apnea, _, _, _, _, _ = model(torch.flip(batch, dims=[2]))
			outputs3_stage, outputs3_apnea, _, _, _, _, _ = model(-batch)
...
def inference_REC_plot(X_concat, Y_concat, THO_concat, ABD_concat, model, device, step_sig, threshold):
	model_input = np.stack([X_concat, Y_concat], axis=0)  # shape (2, N)
	model_input = model_input.reshape(1, 2, -1)  # shape (1, 2, N)
	...
	for i in range(0, model_input.shape[2] - 600 + step_sig, step_sig):
		...
		if segment.shape[2] == 590:
			segments.append(segment)
			tho_segment = (THO_concat[i+5:i+595] - np.mean(THO_concat[i+5:i+595])) / (np.std(THO_concat[i+5:i+595]) + 1e-6)
			abd_segment = (ABD_concat[i+5:i+595] - np.mean(ABD_concat[i+5:i+595])) / (np.std(ABD_concat[i+5:i+595]) + 1e-6)
			tho_segments.append(tho_segment)
			abd_segments.append(abd_segment)
	...
	return np.array(pred_res_stage), np.array(pred_res_apnea), np.array(all_rec_tho), np.array(all_rec_abd), np.array(tho_segments), np.array(abd_segments), np.array(all_x), np.array(all_y)
...
if __name__ == "__main__":
	...
	model_folder_name = f'XY_60s_F1_ws1_wa1_REC0.001'
	config_path = f"/home/jiayu/SleepApnea4Ubicomp_Feb/Code/configs_mtl/{model_folder_name}.yaml"
	...
	for file in sorted(os.listdir(data_folder)):
		...
		# ============ Inference ============
		model = load_model_MTL_REC(fold_idx, model_folder_name, device)
