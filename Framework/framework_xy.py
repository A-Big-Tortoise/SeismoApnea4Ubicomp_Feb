...
if __name__ == "__main__":
	...
	test = False
	if test == False: msg_queue = queue.Queue(maxsize=20)
	...
	if test == False:
		write_db_thread = threading.Thread(target=write_db, args=(dest_client,))
		write_db_thread.daemon = True
		write_db_thread.start()


	# Jiayu
	# start_time = 1770321733633 / 1000
	# end_time = 1770321878664 / 1000

	# Jiahui
	# start_time = 1770322049249 / 1000
	# end_time = 1770322309666 / 1000


	# Zixuan

	# Yida
	# start_time = 1770322576738 / 1000
	# end_time = 1770322874030 / 1000
	...
	while start_time - 60 < end_time:
		...
		json_body.append({
			"measurement": 'sleep',
			"tags": {'location': mac},
			"time": convert_ms_timestamp_to_ny_datetime(int(rr_start_time * 1000)),
			"fields": {'stage_v1': pred_stage_final, 'stage_v4': filtered_stage_lst[-1],
			  'apnea_v4': filtered_apnea_lst[-1], 'apnea_v1': pred_apnea_final}
		})
